...
CUSTOM_NAMESPACE_START(BxDF)
    ...
    half3 SimpleBRDF(CustomLitData customLitData, CustomSurfacedata customSurfaceData, half3 L, half3 lightColor,
                     float shadow)
    {
    ...
    half3 ComplexBRDF(CustomLitData customLitData, CustomSurfacedata customSurfaceData,
                      CustomClearCoatData customClearCoatData, half3 L, half3 lightColor, float shadow)
    {
        float a2 = Common.Pow4(customSurfaceData.roughness);
        ...
        float3 clearCoatTerm = ClearCoatGGX(customClearCoatData.clearCoatRoughness, customClearCoatData.clearCoat, NoH,
                                            NoV, NoL, VoH, clearCoatF);
    ...
    half3 ComplexEnvBRDF(CustomLitData customLitData, CustomSurfacedata customSurfaceData,
                         CustomClearCoatData customClearCoatData, float envRotation, float3 positionWS)
    {
        half NoV = saturate(abs(dot(customLitData.N, customLitData.V)) + 1e-5); //区分正反面
        half3 R = reflect(-customLitData.V, customLitData.N);
        R = Common.RotateDirection(R, envRotation);
        ...
        //ClearCoat
        specularLD = GlossyEnvironmentReflection(R, positionWS, customClearCoatData.clearCoatRoughness,
                                                 customSurfaceData.occlusion);
        specularDFG = EnvBRDFApprox(float3(0.04, 0.04, 0.04), customClearCoatData.clearCoatRoughness, NoV);
        specularOcclusion = GetSpecularOcclusionFromAmbientOcclusion(NoV, customSurfaceData.occlusion,
                                                                     customClearCoatData.clearCoatRoughness);
...
CUSTOM_NAMESPACE_START(DirectLighting)
    half3 SimpleShading(CustomLitData customLitData, CustomSurfacedata customSurfaceData, float3 positionWS,
                        float4 shadowCoord)
    {
    ...
    half3 ComplexShading(CustomLitData customLitData, CustomSurfacedata customSurfaceData,
                         CustomClearCoatData customClearCoatData, float3 positionWS, float4 shadowCoord)
    {
        half3 directLighting = (half3)0;
        #if defined(_MAIN_LIGHT_SHADOWS_SCREEN) && !defined(_SURFACE_TYPE_TRANSPARENT)
        	float4 positionCS = TransformWorldToHClip(positionWS);
        ...
        half3 directLighting_MainLight = (half3)0;
        {
            ...
            #endif
            half shadow = light.shadowAttenuation;
            directLighting_MainLight = BxDF.ComplexBRDF(customLitData, customSurfaceData, customClearCoatData, L,
                                                        lightColor, shadow);
        }
...
CUSTOM_NAMESPACE_START(InDirectLighting)
    ...
    half3 ComplexEnvShading(CustomLitData customLitData, CustomSurfacedata customSurfaceData,
                            CustomClearCoatData customClearCoatData, float envRotation, float3 positionWS)
    {
        half3 inDirectLighting = (half3)0;

        inDirectLighting = BxDF.ComplexEnvBRDF(customLitData, customSurfaceData, customClearCoatData, envRotation,
                                               positionWS);
...
CUSTOM_NAMESPACE_START(PBR)
    half4 SimpleLit(CustomLitData customLitData, CustomSurfacedata customSurfaceData, float3 positionWS,
    ...
    half4 ComplexLit(CustomLitData customLitData, CustomSurfacedata customSurfaceData,
                     CustomClearCoatData customClearCoatData, float3 positionWS, float4 shadowCoord, float envRotation)
    {
        float3 albedo = customSurfaceData.albedo;
        customSurfaceData.albedo = lerp(customSurfaceData.albedo, float3(0.0, 0.0, 0.0), customSurfaceData.metallic);
        customSurfaceData.specular = lerp(float3(0.04, 0.04, 0.04), albedo, customSurfaceData.metallic);
        half3x3 TBN = half3x3(customLitData.T, customLitData.B, customLitData.N);
        customLitData.N = normalize(mul(customSurfaceData.normalTS, TBN));
        customSurfaceData.specular = lerp(customSurfaceData.specular,
                                          ConvertF0ForClearCoat15(customSurfaceData.specular),
                                          customClearCoatData.clearCoat);
        #if defined(_CLEARCOAT_OFF)
            customSurfaceData.specular = lerp(float3(0.04,0.04,0.04),albedo,customSurfaceData.metallic);
        #endif
        //SSAO
        ...
        //DirectLighting
        half3 directLighting = DirectLighting.ComplexShading(customLitData, customSurfaceData, customClearCoatData,
                                                             positionWS, shadowCoord);

        //IndirectLighting
        half3 inDirectLighting = InDirectLighting.ComplexEnvShading(customLitData, customSurfaceData,
                                                                    customClearCoatData, envRotation, positionWS);
        return half4(directLighting + inDirectLighting, 1);
    }
